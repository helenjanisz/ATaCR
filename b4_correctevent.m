...
if exist(badstalist,'file')
	badstas = textread(badstalist,'%s');
else
    badstas = [];
end
if exist(badhorzlist,'file')
	badhorz = textread(badhorzlist,'%s');
else
    badhorz = [];
end
if exist(badpreslist,'file')
...
for ie = 1:length(event_filename) 
    ...
    for ista = 1:stas %loop through stations for given event
        ...
        idx1 = strfind(event_info_dir(ista).name,'_');
        idx2 = strfind(event_info_dir(ista).name,'.mat');
        ...
        fprintf('%s/%s/%s\n',inpath_event,event_filename(ie).name,event_info_dir(ista).name); %display file name for check, can turn off
        ...
        if tf_op == 1
            ...
            figoutpath = sprintf('%s/CORREVENTS/%s',FIGdir,netsta);
        elseif tf_op == 2
            outpath = sprintf('%s/CORRSEISAVTF/%s/',OUTdir,netsta);
            figoutpath = sprintf('%s/CORREVENTS/%s',FIGdir,netsta);
        end
		
        if ~isoverwrite && exist(sprintf('%s/%s_%s_corrseis.mat',outpath,netsta,eventid),'file')==2
        ...
        if exist(trans_filename,'file')
            ...
            goodZ=1;
            if ~isempty(badstas)
            if ~isempty(find(contains(badstas,netsta), 1))
                disp('Vertical Bad.')
                goodZ=0; %if pressure is marked as bad, turn good flag off
            end
            end
            if ~isempty(badpres)
            if ~isempty(find(contains(badpres,netsta), 1))
                disp('Pressure Bad.')
                goodP=0; %if pressure is marked as bad, turn good flag off
            end
            end
            if ~isempty(badhorz)
            if ~isempty(find(contains(badhorz,netsta), 1))
                disp('Horizontal Bad.')
